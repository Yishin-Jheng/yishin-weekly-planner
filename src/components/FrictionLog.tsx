...
import { TIME_SLOTS, FRICTION_TAGS, getTagLabel, getTimeSlotLabel } from '../utils/frictionConstants'
  ...
  return (
    <div className="card">
      <div className="friction-header">
        <div className="card-title" style={{ margin: 0 }}>摩擦日誌</div>
        {!isOpen && (
      ...
      {isOpen && (
        <div className="friction-form">
          ...
          <div className="friction-form-actions">
            ...
            <button className="friction-cancel-btn" onClick={() => setIsOpen(false)}>
              取消
            </button>
          </div>
        </div>
      ...
      <div className="friction-list">
        {entries.length === 0 && !isOpen ? (
          <div className="friction-empty">這週還沒有記錄，狀態很好？還是忘了記？</div>
        ) : (
          entries.map(entry => (
            <div key={entry.id} className="friction-entry">
              <div className="friction-entry-left">
                <span className="friction-entry-date">{formatDate(entry.date)}</span>
                {entry.timeSlot && (
                  <span className="friction-entry-slot">{getTimeSlotLabel(entry.timeSlot)}</span>
                )}
              </div>
              <div className="friction-entry-body">
                {entry.tags.length > 0 && (
                  <div className="friction-entry-tags">
                    {entry.tags.map(t => (
                      <span key={t} className="friction-tag-display">{getTagLabel(t)}</span>
                    ))}
                  </div>
                )}
                {entry.note && (
